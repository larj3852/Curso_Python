...
class CheckBoxWindow(QWidget):
    ...
    def displayCheckBoxes(self):
        ...
        # Inicializar checboxes
        morning_cb = QCheckBox("Morning [8 AM-2 PM]", self) # text, padre
        morning_cb.move(20, 80)
        #morning_cb.toggle() # Si se quiere que el checkbox se inicialice marcado
    ...
    def printToTerminal(self, state): # pass state of checkbox
        ...
        '''
        #Identifica las señales de cada boton en caso de ser cambiados de estado
        sender = self.sender()  
        if state == Qt.Checked:
